...
import numpy as np
import sys
from architectures import AE_model128 as ae
import time
import random
...
num_patches_per_section = 102400 #400384  #500224
...
optimizer = optim.Adam(AE.parameters(), lr=1e-3)  # lr is the default

# -------------------------------------------------
checkpoint_path_save = os.path.join(folder_save, ae_name + "_checkpoint2.pth")
...
for epoch in range(num_epochs):
    ...
    # train_loss /= len(data_loader_training.dataset)
    # train_loss_per_epoch.extend(train_loss_per_batch)

    # val_loss /= len(data_loader_testing.dataset)
    # val_loss_per_epoch.extend(val_loss_per_batch)
...
if not stop_all:
    print("Training complete, y epochs reached")
    torch.save(best_model, save_path)
    print(f"best model saved to {save_path}")

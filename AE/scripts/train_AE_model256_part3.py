...
for epoch in range(num_epochs):
    ...
    if stop_all:
        ...
        np.savetxt(folder_save + ae_name + '_train_loss_part3.txt', np.array(train_loss_1_8))
        np.savetxt(folder_save + ae_name + '_val_loss_part3.txt', np.array(val_loss_1_8))
...
if not stop_all:
    print("Training complete, y epochs reached, not best model achieved")
    torch.save(best_model, save_path)
    print(f"best model saved to {save_path}")
